...
# Attempt to import core logic from monorepo backend if available;
# otherwise fall back to self-contained standalone definitions for single .exe distribution.
try:
...
def find_config_path(cli_path: str | None = None) -> Path | None:
    ...
    for name in ["agent.env", "connector.env", ".env"]:
        ...
        if p.is_file():
            return p
    # Search script / executable directory
...
def main():
    ...
    # Resolution hierarchy: CLI arg -> Environment variable -> agent.env file
    ...
    # Trust server certificate resolution
    ...
    # Interactive prompts for missing required parameters if running interactively
    if not url:
        if is_interactive:
    ...
    # Optionally save settings to agent.env for easy subsequent runs
